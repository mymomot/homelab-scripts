...
MAX_ALERTS_PER_DAY=3         # D — cap anti-spam : max 3 alertes/24h même sujet (2026-04-24)
ROLLING_WINDOW=86400         # 24h rolling window pour le compteur
...
# --- 3. Dédup : cap horaire (1h) + cap journalier (3/24h) ---
# Format state file : "EPOCH|COUNT" — COUNT = nb alertes dans ROLLING_WINDOW
# Rétro-compatible avec format legacy "EPOCH" seul.
COUNT=0
if [[ -f "$STATE_FILE" ]]; then
    LAST_LINE=$(cat "$STATE_FILE")
    LAST_ALERT_EPOCH="${LAST_LINE%|*}"
    LAST_COUNT="${LAST_LINE#*|}"
    # Legacy format (pas de |) → fallback count=1
    [[ "$LAST_COUNT" == "$LAST_LINE" ]] && LAST_COUNT=1
    DEDUP_AGE=$(( $(date +%s) - LAST_ALERT_EPOCH ))

    # Reset compteur si dernière alerte > ROLLING_WINDOW (24h)
    if (( DEDUP_AGE > ROLLING_WINDOW )); then
        COUNT=0
        log "reset compteur — dernière alerte il y a ${DEDUP_AGE}s (> ${ROLLING_WINDOW}s rolling window)"
    else
        COUNT=$LAST_COUNT
    fi

    # Cap 1 : alerte trop récente (< 1h)
    if (( DEDUP_AGE < DEDUP_WINDOW )); then
        log "SKIP — déjà alerté il y a ${DEDUP_AGE}s (< ${DEDUP_WINDOW}s)"
        exit 0
    fi

    # Cap 2 : quota journalier atteint (> MAX_ALERTS_PER_DAY en 24h)
    if (( COUNT >= MAX_ALERTS_PER_DAY )); then
        log "SKIP — quota atteint ${COUNT}/${MAX_ALERTS_PER_DAY} alertes en ${ROLLING_WINDOW}s"
        exit 0
    fi
fi
...
# Mémorise l'alerte pour dédup (format EPOCH|COUNT)
NEW_COUNT=$(( COUNT + 1 ))
echo "$(date +%s)|${NEW_COUNT}" > "$STATE_FILE"
log "DONE — alerte ${NEW_COUNT}/${MAX_ALERTS_PER_DAY} dans la fenêtre rolling ${ROLLING_WINDOW}s"
