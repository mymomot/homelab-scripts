...
- **vault-mem R/W** via MCP : search, write (section `debug` tag `bigbrother-analysis`), trace
...
3. **Historique** — comparer avec analyses précédentes :
   ```
   vault_search query="<service/pattern clé>" section=debug tag=bigbrother-analysis limit=10 caller=bigbrother
   ```
...
6. **Persistance** :
   ```
   vault_write section=debug tag=bigbrother-analysis author="Claude Code" caller=bigbrother
